<%-- 
    Document   : leaveApproverMaintanance
    Created on : 25 Sep, 2019, 3:06:09 PM
    Author     : EMINENT
--%>

<%@page import="com.eminent.leaveUtil.LeaveApproverMaintenance"%>
...
<body>
    ...
    <%
        ...
        if (roleId == 1) {
    %> 
    <jsp:useBean id="tmc" class="com.eminent.leaveUtil.LeaveApproverMaintenanceController"/>
    <%tmc.setAll(request);%>
    <div style="width: 100%;background-color: #C3D9FE;font-weight: bold;">Centralized Leave Approver Maintenance </div>
    <br/>
    ...
    <form name="theForm" id="theForm" method="post" action="<%=request.getContextPath()%>/leave/leaveApproverMaintanance.jsp" onsubmit="return validate();">
        <div >
            <label style=" padding: 10px;font-weight: bold;">Requesters</label>
            <%if (tmc.getId() == null && tmc.getLeaveApproverMaintenance() == null) {%>
            ...
            <%} else {%>
            <input type="text" class="custom-select" value="<%=tmc.getEminentUsers().get(MoMUtil.parseInteger(tmc.getId(), 0))%>" readonly=""/>
            <input type="hidden" name="selectItemrequesters"  id="requesters" value="<%=tmc.getId()%>"/>                    
            <%}%>
        </div>
        <br/>
        <%if (tmc.getId() == null) {%>
        <div>
            <label style = " padding: 10px;font-weight: bold;" > Approver :</label>

            <select class="custom-select"  name="approver" id="approver1">
        ...
        <%} else {
            for (LeaveApproverMaintenance lam : tmc.getLeaveApproverMaintenance()) {
        %>
        <div>
            <br/>
            <label style = " padding: 10px;font-weight: bold;" > Approver <%=lam.getApprovalLevel()%>:</label>
            <select class="custom-select"  name="approver" id="approver<%=lam.getApprovalLevel()%>">
                <option value="0"  selected="">Select</option>
                <%for (Map.Entry<Integer, String> team : tmc.getEminentUsers().entrySet()) {
                        if (lam.getApprover() == team.getKey().intValue()) {%>
            ...
            <img src="<%=request.getContextPath()%>/images/remove.gif"  class="delete" id="<%=lam.getApprovalId()%>"></img>
        </div>
        <%}
            }%>
        <div class="addLevelArea">

        </div>
        <br/>
        <div >

            <input type="button" name="button" id='button' class="addLevel" value="Add Level" /> <input type="submit" name="submit" id="submit"  value="Submit"> </div>
    </form>
    <%if (tmc.getApproverRoles() != null && !tmc.getApproverRoles().isEmpty()) {%>
    <table width=100% id="user" class="tablesorter">
        ...
        <thead>
            ...
            <tr bgColor="#C3D9FF" height="9">
                <th class="filter-select filter-match" data-placeholder="Select a Requester"><font><b>Requester</b></font></th>
                <th class="filter-select filter-match" data-placeholder="Select a Level"> <font><b>Level</b></font></th>
                <th class="filter-select filter-match" data-placeholder="Select a Approver"><font><b>Approver</b></font></th>
            </tr>
        </thead>
        <tbody>
            <%for (LeaveApproverMaintenance u : tmc.getApproverRoles()) {
                    if (tmc.getEminentUsers().containsKey(u.getRequester())) {%>
            <tr >
                <td ><a href="<%=request.getContextPath()%>/leave/leaveApproverMaintanance.jsp?id=<%=u.getRequester()%>"><%=tmc.getEminentUsers().get(u.getRequester())%> </a></td>
                <td ><%=u.getApprovalLevel()%>   </td>
                <td ><%=tmc.getEminentUsers().get(u.getApprover())%>   <img src="<%=request.getContextPath()%>/images/remove.gif"  class="delete" id="<%=u.getApprovalId()%>"></img></td>
            </tr>
            <%}
                }%>
        </tbody>
    ...
    <script src="<%=request.getContextPath()%>/javaScript/pager-custom-controls.js" type="text/javascript"></script>    <script type="text/javascript">
        ...
        $(document).on("click", ".addLevel", function () {
            var counter = $('select[name="approver"]').length;

            var info = "<div><br/><label style = ' padding: 10px;font-weight: bold;' > Approver " + (counter + 1) + " :</label><select class='custom-select'  name='approver' id='approver" + (counter++) + "' >  ";
            $("#approver1 option").each(function () {
                info = info + "<option value='" + $(this).val() + "' >" + $(this).text() + "</option>";
            });
            info = info + "</select><img src='<%=request.getContextPath()%>//images/remove.gif'  class='delete' id='0'></img></div>";
            $(".addLevelArea").append(info);
        });
        function validate() {
            $(".error1").remove();
            var count = 0;
        ...
        $(document).on("click", ".delete", function () {
            ...
            if (confirm("Do you want to delete?")) {
                ...
                if (id === "0") {
                    ...
                    $.ajax({
                        url: '<%=request.getContextPath()%>/leave/deleteLeaveApprover.jsp',
                        ...
                        success: function (responseText, statusTxt, xhr) {
                            if (statusTxt == "success") {
                                ...
                                if (result === "success") {
                                    obj.parent().remove();
                                    $('#user').trigger('update');
                                    alert("Leave approver deleted successfully");
                                } else {
                                    alert(result);
                                }
                            }
