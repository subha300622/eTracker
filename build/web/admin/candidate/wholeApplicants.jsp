<%-- 
    Document   : wholeApplicants
    Created on : Mar 11, 2014, 10:48:06 AM
    Author     : E0288
--%>

<%@page import="com.eminentlabs.erm.Applicants"%>
...
<%
    ...
    Logger logger = Logger.getLogger("wholeApplicants");
...
<html>
    ...
    <script type="text/javascript" src="<%= request.getContextPath()%>/javaScript/ERMFileAttach.js?test=20012021"></script>          
...
<body>
    ...
    <jsp:useBean id="ap" class="com.eminentlabs.erm.Applicants"></jsp:useBean>
    <%
        ap.setAll(request);
    %>
    <form name="issueSearch" onsubmit="return fun(this);" action="<%= request.getContextPath()%>/admin/candidate/viewdetails.jsp">
        ...
        <tr>
            <td>
                <%if (ap.getUsCount() > 0) {
                        if (ap.getaStatus() == 0) {
                %>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=0" style="font-weight: bold;"><%=ERMUtil.ermApplicantStatus().get(0)%>(<%=ap.getUsCount()%>)</a> &nbsp;&nbsp;&nbsp;&nbsp;
                <%} else {%>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=0" ><%=ERMUtil.ermApplicantStatus().get(0)%>(<%=ap.getUsCount()%>)</a> &nbsp;&nbsp;&nbsp;&nbsp;
                <%}
                    }
                    if (ap.getReCount() > 0) {
                        if (ap.getaStatus() == 3) {%>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=3" style="font-weight: bold;"> <%=ERMUtil.ermApplicantStatus().get(3)%>(<%=ap.getReCount()%>)</a> &nbsp;&nbsp;&nbsp;&nbsp;
                <%} else {%>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=3" > <%=ERMUtil.ermApplicantStatus().get(3)%>(<%=ap.getReCount()%>)</a> &nbsp;&nbsp;&nbsp;&nbsp;
                <%}
                    }
                    if (ap.getHoldCount() > 0) {
                        if (ap.getaStatus() == 7) {
                %>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=7" style="font-weight: bold;"><%=ERMUtil.ermApplicantStatus().get(7)%>(<%=ap.getHoldCount()%>)</a> &nbsp;&nbsp;&nbsp;&nbsp;
                <%} else {%>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=7" ><%=ERMUtil.ermApplicantStatus().get(7)%>(<%=ap.getHoldCount()%>)</a> &nbsp;&nbsp;&nbsp;&nbsp;
                <%}
                    }
                    if (ap.getScCount() > 0) {
                        if (ap.getaStatus() == 1) {
                %>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=1" style="font-weight: bold;"><%=ERMUtil.ermApplicantStatus().get(1)%>(<%=ap.getScCount()%>)</a>   &nbsp;&nbsp;&nbsp;&nbsp;
                <%} else {%>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=1" ><%=ERMUtil.ermApplicantStatus().get(1)%>(<%=ap.getScCount()%>)</a>   &nbsp;&nbsp;&nbsp;&nbsp;
                <% }
                    }
                    if (ap.getShCount() > 0) {
                        if (ap.getaStatus() == 2) {
                %>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=2" style="font-weight: bold;"><%=ERMUtil.ermApplicantStatus().get(2)%>(<%=ap.getShCount()%>)</a>&nbsp;&nbsp;&nbsp;&nbsp;
                <%} else {%>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=2" ><%=ERMUtil.ermApplicantStatus().get(2)%>(<%=ap.getShCount()%>)</a>&nbsp;&nbsp;&nbsp;&nbsp;
                <%}
                    }
                    if (ap.getJoCount() > 0) {
                        if (ap.getaStatus() == 4) {
                %>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=4" style="font-weight: bold;"><%=ERMUtil.ermApplicantStatus().get(4)%>(<%=ap.getJoCount()%>)</a>&nbsp;&nbsp;&nbsp;&nbsp;
                <%} else {%>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=4" ><%=ERMUtil.ermApplicantStatus().get(4)%>(<%=ap.getJoCount()%>)</a>&nbsp;&nbsp;&nbsp;&nbsp;
                <%}
                    }
                    if (ap.getOffCount() > 0) {
                        if (ap.getaStatus() == 5) {
                %>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=5" style="font-weight: bold;"><%=ERMUtil.ermApplicantStatus().get(5)%>(<%=ap.getOffCount()%>)</a> &nbsp;&nbsp;&nbsp;&nbsp;
                <%} else {%>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=5" ><%=ERMUtil.ermApplicantStatus().get(5)%>(<%=ap.getOffCount()%>)</a> &nbsp;&nbsp;&nbsp;&nbsp;
                <%}
                    }
                    if (ap.getDisCount() > 0) {
                        if (ap.getaStatus() == 6) {
                %>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=6" style="font-weight: bold;"><%=ERMUtil.ermApplicantStatus().get(6)%>(<%=ap.getDisCount()%>)</a> &nbsp;&nbsp;&nbsp;&nbsp;
                <%} else {%>
                <a href="<%=request.getContextPath()%>/admin/candidate/wholeApplicants.jsp?applicantStatus=6" ><%=ERMUtil.ermApplicantStatus().get(6)%>(<%=ap.getDisCount()%>)</a> &nbsp;&nbsp;&nbsp;&nbsp;
                <%}
                    }
                %>
            </td>
    ...
    <table width=100% id="user" class="tablesorter">
        ...
        <tbody>
            <% for (Applicants ep : ap.getApllicantList()) {%>
            <tr >

                <td><a href="<%=request.getContextPath()%>/admin/candidate/viewdetails.jsp?apid=<%=ep.getApplicantId()%>"><%=ep.getApplicantId()%></a></td>
                <td ><%=ep.getName()%>
                    <%-- <td class="imagePopup"  onclick="showImage('<%=request.getContextPath()%>/Etracker_AttachedFiles/userPhotos/<%=ep.getPhotoPath()%>', '<%=ep.getName()%>', '<%=ep.getApplicantId()%>');"><%=ep.getName()%>--%>
                    <%if (ep.getIsFake() == 1) {%>
                ...
                <td><%=ep.getSapExp()%></td>
                <td id="filesIssue"> <A	onclick="viewResume('<%=ep.getApplicantId()%>');" href="#">Resume</A></td>         
                <td><%=ep.getAssignedTo()%></td></tr>
        ...
        <tfoot>
            <tr>
                <td colspan="12" style="background-color: #c3d9ff;">
                    <div class="pager"><nav class="left"> # per page: <a href="#">10</a> | <a href="#" >25</a> | <a href="#">50</a>|<a href="#" class="current">100</a>
                        </nav> 
                        <nav class="right"> <span class="prev"> <img
                                    src="<%=request.getContextPath()%>/images/prev.png" /> Prev&nbsp; </span> <span
    ...
    <div id="overlay"></div>
    <div id="MDAVpopup" class="popup" style="height: 300px;width: 300px">
