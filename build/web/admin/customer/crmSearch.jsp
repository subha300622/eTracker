...
<html>
    ...
    <script type="text/javascript">
        ...
        function getCRMCreatedBy() {
            xmlhttp = createRequest();
            if (xmlhttp !== null) {
                var crmType = document.getElementById('crmType').value;
                xmlhttp.open("GET", "/eTracker/admin/customer/getCRMCreatedUsers.jsp?crmType=" + crmType + "&rand=" + Math.random(1, 10), false);
                xmlhttp.onreadystatechange = callBackCRMCreatedBy;
                xmlhttp.send(null);
            }
        }
        function callBackCRMCreatedBy()
        {
            if (xmlhttp.readyState === 4)
            {
                if (xmlhttp.status === 200)
                {
                    var result = xmlhttp.responseText.split(";");
                    var results = result[1].split(",");
                    objLinkList = eval("document.theForm.createdby");
                    objLinkList.length = 0;
                    for (i = 0; i < results.length - 1; i++) {
                        var k = results[i];
...
<body onload="javascript:getCRMCompany();
        ...
        <table cellpadding="0" cellspacing="0" width="100%">
            <tr border="2" bgcolor="#E8EEF7">
                <td bgcolor="#E8EEF7" border="1" align="left"><font size="4"
                                                                    COLOR="#0000FF"> <b> CRM Search </b></font><FONT SIZE="3"
    ...
    <form name="theForm" onsubmit="return val(this);"  action="<%=request.getContextPath()%>/admin/customer/crmSearchResults.jsp">
        <table width="100%" bgColor="#E8EEF7" cellspacing="2" cellpadding="2" align="center">
            <tr>
                <td width="25%">
                    <b>CRM Type</b>
                </td>
            ...
            <tr>
                <td width="25%">
                    <select id="crmType" name="crmType" onchange="javascript:getCRMArea();
            ...
            <tr >
                <td width="25%" class="vendor-erp">
                    <select name="erp" id="erp" size="1">
                        <option value="" disabled selected>All Information</option>
