...
<jsp:useBean id="amac" class="com.eminent.issue.controller.ApmModuleAttachmentController"></jsp:useBean>
<html>
...
<%List<ApmModuleAttachment> list = amac.viewDocuments(request);
    SimpleDateFormat sdf = new SimpleDateFormat("dd-MMM-yyyy HH:mm:ss");
    HashMap modules = new HashMap(0);
    if (request.getParameter("pId") != null) {
        modules = GetProjects.getModules(request.getParameter("pId"));
    }
   %>
<body>
<table style="width: 100%;" class="tablesorter" id="documentTable">
    <thead style="height: 26px;font-weight: bold;color:#000;background-color: #C3D9FE;">
        ...
        <th class="filter-select filter-match" data-placeholder="All">Module</th>
        <th>File Name</th>
    </thead>
    <%int i = 0;
        for (ApmModuleAttachment apmModuleAttachment : list) {
            if (i % 2 == 0) {%>
        ...
        <td class="background"><%=modules.get(String.valueOf(apmModuleAttachment.getModuleid()))%></td>
