<%-- 
    Document   : maintainSAPConfig
    Created on : 18 May, 2020, 12:59:40 PM
    Author     : vanithaalliraj
--%>


<%@page import="com.eminent.server.SapConfig"%>
...
<!DOCTYPE html>
<%
    Logger logger = Logger.getLogger("maintainSAPConfig");
...
<html>
    ...
    <script>
        function callServer(theForm)
        {
            ...
            document.theForm.action = '/eTracker/admin/project/maintainSAPConfig.jsp?pid=' + pid + "&serverId=" + x + "&companyCode=" + companyCode;
        ...
        function call()
        {
            ...
            var pid = document.getElementById("pid").value;
            document.projectForm.action = '/eTracker/admin/project/maintainSAPConfig.jsp?pid=' + pid;
...
<!--                <a href="<%=request.getContextPath()%>/UserBPM/dashboardForCompany.jsp?pid=<%=pid%>">View Test Map Dashboard</a>&nbsp;&nbsp;&nbsp;-->
    ...
    <form name="monitorForm" method="post" action="<%=request.getContextPath()%>/admin/project/maintainSAPConfig.jsp">   

        <table width="70%" id="monitor" bgColor=#E8EEF7 border="0" align="center">
            <tr>
                <th>   <input type="hidden" name="pid" id="pid" value="<%=pid%>"/>System</th><th>OS</th><th>ERP</th><th>OHP</th><th>DB</th></tr>          
                    <%for (SapConfig ss : sm.getSapConfigs()) {%>
            <tr>
                <td><input type="hidden" name="matrixId" value="<%=ss.getmId()%>"><input type="hidden" name="lid" value="<%=ss.getId()%>"><%=ss.getSystemName()%></td>
                <td><input type="text" name="os<%=ss.getmId()%>" value="<%=ss.getOsName() == null ? "" : ss.getOsName()%>"></td>
                <td><input type="text" name="erp<%=ss.getmId()%>" value="<%=ss.getErpVersion() == null ? "" : ss.getErpVersion()%>"></td>
                <td><input type="text" name="ehp<%=ss.getmId()%>" value="<%=ss.getEhpVersion() == null ? "" : ss.getEhpVersion()%>"></td>
                <td><input type="text" name="db<%=ss.getmId()%>" value="<%=ss.getDbVersion() == null ? "" : ss.getDbVersion()%>"></td>
            </tr>
            <%   }%>
        </table>
