<%-- 
    Document   : displayIssueAjax
    Created on : 9 May, 2016, 10:59:34 AM
    Author     : admin
--%>
<%@page import="java.util.HashMap"%>
<%@page import="dashboard.controller.MydashboardController"%>
...
<html>
    ...
    <body>
        ...
        <%
          List<IssueFormBean>  list=  viewBPM.getTestScriptIssues(Integer.parseInt(request.getParameter("tsId")));
          HashMap<Integer, String> mem= GetProjectMembers.showUsers();
        ...
        <div class="tablecontent">
            <div>The below list shows <%=list.size()%> issues</div>
            ...
            <TABLE width="100%" id="filtersort" class="tablesorter">
                <thead>
                    <TR style="background-color:#C3D9FF">
                        ...
                        <td width="11%"><font><b>Issue No</b></font></td>
                                                <td width="1%" class="filter-false" TITLE="Priority"><font><b>P</b></font></td>
                        ...
                        <td width="12%" class="filter-select filter-match" data-placeholder="Select a Module" ><font><b>Status</b></font></td>
                        ...
                        <td width="9%" ><font><b>Refer</b></font></td>
                ...
                <tbody>
                    ...
                    <%      int i = 0;
                        for (IssueFormBean issueFormBean : list) {
                            i++;
                            if ((i % 2) != 0) {
                    %>
                    ...
                    <tr class="zebraline" height="21">
                        ...
                        %>
                         ...
                         <%
                                    if (issueFormBean.getSeverity().equals("S1- Fatal")) {
                                %>
                                <td   width="1%" style="background-color:#FF0000; " bgcolor="#FF0000"></td>
                                <%
                                } else if (issueFormBean.getSeverity().equals("S2- Critical")) {
                                %>
                                <td   width="1%" style="background-color:#DF7401; " bgcolor="#DF7401"></td>
                                <%
                                } else if (issueFormBean.getSeverity().equals("S3- Important")) {
                                %>
                                <td  width="1%" style="background-color:#F7FE2E; " bgcolor="#F7FE2E"></td>
                                <%
                                } else if (issueFormBean.getSeverity().equals("S4- Minor")) {
                                %>
                ...
                <td class="background"  title="<%=issueFormBean.getType()%>"><%--<a  href="<%= request.getContextPath()%>/admin/dashboard/UpdateIssueview.jsp?issueNo=<%=issueFormBean.getIssueId()%>">--%><a target="_blank" href="<%=request.getContextPath()%>/viewIssueDetails.jsp?issueid=<%=issueFormBean.getIssueId()%>" style="visibility: visible"><%= issueFormBean.getIssueId()%></a></td>

           
                <td class="background"><font
                        face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><%= issueFormBean.getPriority().substring(0, 2)%></font></td>
                ...
                %>
                <td class="background" title="Last Modified On <%=issueFormBean.getModifiedOn()%>"><font face="Verdana, Arial, Helvetica, sans-serif"
                ...
                <td class="background"><font
                        face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><%= mem.get(MoMUtil.parseInteger(issueFormBean.getCreatedBy(), 0))%>
                    </font></td>
                ...
                %>
                <td class="background"><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"> <a onclick="viewFileAttachForIssue('<%=issueFormBean.getIssueId()%>');" href="#;">View Files(<%=issueFormBean.getFilecount() %>)</a></font></td>
                    <%
                    } else {
                    %>
                <td class="background"><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000">No Files</font></td>
                    ...
                    %>
                <td class="background" title="<%=issueFormBean.getAge()%>"><%=issueFormBean.getLastAssigneeAge()%></td>
